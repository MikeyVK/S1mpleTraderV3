...
**In Scope:** Presentation configuration and alignment, generic inline-sequence and collection rendering, final UTF-8 byte limiting, TextPresenter integration, presentation.yaml rollout, representative behavioral tests, and deletion of the two obsolete documentation-test modules.
...
1. [Approved Research v1.2](research.md)
2. [Approved Design v1.1](design.md)
...
Five coherent cycles establish validated configuration, generic collection presentation, UTF-8-safe limiting, end-to-end presenter rollout, and mechanical removal of obsolete documentation tests. Cycles 1–4 change durable behavior and use RED → GREEN → REFACTOR. Cycle 5 is test maintenance and deliberately creates no artificial RED test.

The cycles preserve the Approved Strategy: complete DTOs and resources remain unchanged, presentation stays configuration-driven, DTO order is retained, and the only universal text limit is 8,000 UTF-8 bytes.
...
**Goal:** Establish frozen configuration contracts and fail-fast alignment for recursive collections, inline scalar sequences, item limits, omission wording, truncation wording, and the global byte ceiling.
...
- RED: focused tests fail because the new schema/alignment behavior is absent.
...
Exit only when valid configuration loads, each invalid contract fails with actionable tool/path context, and no tool-name logic was introduced. Stop if Pydantic cannot express the approved invariants without changing the Design or DTO boundary.
...
**Goal:** Compose the generic services in TextPresenter and activate all approved presentation declarations without changing DTO or resource contracts.
...
| C_INTEGRATION.2 | All thirteen audited collection tools have valid max_items and collection declarations. |
...
| Risk | Mitigation |
|---|---|
| Configuration and renderer evolve out of step | Cycle 1 establishes fail-fast alignment; Cycle 4 validates the complete matrix. |
