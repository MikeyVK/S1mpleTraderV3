...
# Config-First PSE Architecture ÔÇö Implementation Planning
...
Break down the design decisions from design.md (AÔÇôJ) into six ordered, independently-testable implementation cycles. Each cycle has explicit stop/go criteria before the next cycle begins. Ordered for risk reduction: foundations and renames first, core abstractions second, then config layer, tool layer integration, enforcement, and deliverables tooling last.

## Scope

**In Scope:**
All components from design.md: phase_contracts.yaml, enforcement.yaml, deliverables.json, AtomicJsonWriter, PhaseContractResolver, StateRepository, EnforcementRunner, IStateReader/IStateRepository, PSE refactor, GitConfig.extract_issue_number(), tddÔåÆimplementation rename, projects.json abolishment
...
Read these first:
1. design.md APPROVED ÔÇö all AÔÇôJ decisions finalized
...
**Design decisions:** H1ÔÇôH4, G1ÔÇôG2, I3, C2ÔÇôC4

**Goal:** Eliminate dead code and rename all moving parts before new abstractions are introduced. Flag-day: `tdd` ÔåÆ `implementation`, `workflow_config.py` deleted, `GitConfig.extract_issue_number()` added, `projects.json` abolished.
...
**Stop/Go:** Ô£à Go to Cycle 2 only if all three success criteria pass.

---

### Cycle 2: StateRepository + BranchState + AtomicJsonWriter (E + B3)

**Design decisions:** E1ÔÇôE4, B3
...
**Stop/Go:** Ô£à Go to Cycle 3 only if Pyright strict passes and InMemoryStateRepository is the default in all PSE unit tests.

---

### Cycle 3: phase_contracts.yaml loader + PhaseContractResolver (A + D + G3)

**Design decisions:** A1, A3, A5, A6, D1ÔÇôD5, G3
...
**Stop/Go:** Ô£à Go to Cycle 4 only if Fail-Fast test and Pyright strict both pass.

---

### Cycle 4: Tool layer integration + PSE.get_state() + legacy param drop (J)

**Design decisions:** J1ÔÇôJ4
...
**Stop/Go:** Ô£à Go to Cycle 5 only if grep check and Pyright strict both pass.

---

### Cycle 5: enforcement.yaml + EnforcementRunner (F + F3 + F5)

**Design decisions:** F1ÔÇôF5
...
**Success Criteria:**
- End-to-end test: `transition_phase` triggers post-hook ÔåÆ `state.json` committed automatically
...
**Stop/Go:** Ô£à Go to Cycle 6 only if both end-to-end tests pass.

---

### Cycle 5.1: transition_tools refactor ÔÇö shared base + enforcement hooks + DIP/DRY fixes (F6)

**Design decisions:** F6.1ÔÇôF6.5

**Goal:** Bring `TransitionCycleTool` and `ForceCycleTransitionTool` in line with the C1ÔÇôC5 refactoring. Three violations introduced before the refactoring are corrected in a single focused cycle: DIP (direct settings access in execute), DRY (private `_extract_issue_number()` instead of `GitConfig`), and F3 gap (`enforcement_event` missing ÔåÆ cycle state never auto-committed).
...
- Dispatch E2E test: `transition_cycle` triggers post-hook ÔåÆ `state.json` committed automatically (F6.4, analogous to `transition_phase` test in `test_server.py`)
...
**Stop/Go:** Ô£à Go to Cycle 6 only if grep checks, E2E dispatch test, and Pyright strict all pass.

---

### Cycle 6: deliverables.json tools + state.json git-tracked (B1 + B2 + B4 + B5)

**Design decisions:** B1ÔÇôB5
...
- KPIs 1ÔÇô20 in `research_config_first_pse.md` all verifiable

**Stop/Go:** Ô£à KPIs 1ÔÇô20 all verifiable ÔåÆ open PR.

---

## Risks & Mitigation

- **Risk:** Cycle 1 ÔÇö `tdd` ÔåÆ `implementation` rename breaks active `state.json` files on other branches
...
- **Risk:** Cycle 2 ÔÇö PSE state refactor introduces regression in read/write path
...
- **Risk:** Cycle 3 ÔÇö `phase_contracts.yaml` schema mismatch with existing `.st3/config/` YAML files
...
- **Risk:** Cycle 4 ÔÇö legacy `phase=` param removal breaks undiscovered callers outside `mcp_server/tools/`
...
- **Risk:** Cycle 5 ÔÇö dispatch-level `EnforcementRunner` injection increases server startup complexity
...
- **Risk:** Cycle 6 ÔÇö `.gitignore` removal of `state.json` affects all branches simultaneously
...
- After Cycle 1: codebase free of tdd, projects.json, workflow_config.py, old class names ÔÇö green test suite
- After Cycle 2: PSE decoupled from filesystem; state I/O behind IStateRepository ÔÇö Pyright strict passes
- After Cycle 3: phase gates config-driven; PhaseContractResolver independently testable ÔÇö Fail-Fast validated
...
- After Cycle 6: deliverables.json tooling complete; KPIs 1ÔÇô20 in research_config_first_pse.md all verifiable ÔÇö ready for PR

## Appendix ÔÇö `save_planning_deliverables` Payload

> **Doel:** ├ë├®n-op-├®├®n persisteerbare payload. Geen interpretatiestap vereist.
> Kopieer de JSON-block hieronder direct naar de `save_planning_deliverables` tool-aanroep.
>
> **Expliciete keuze over fase-entries:**
> - `design` ÔåÆ **inbegrepen**: design.md is APPROVED; twee structurele checks.
> - `validation` ÔåÆ **inbegrepen**: ├®├®n check op aanwezigheid KPI-sectie na Cycle 6.
> - `documentation` ÔåÆ **inbegrepen**: ├®├®n check op SCAFFOLD-header planning.md.
>
> **`validates`-types gebruikt:** `file_exists`, `contains_text`, `absent_text` ÔÇö allemaal met verplicht `file`-veld, gevalideerd door `validate_spec` en `DeliverableChecker`.
  ...
  "planning_deliverables": {
      ...
      "cycles": [
        {
          ...
          "deliverables": [
            {
              "id": "D1.1",
              "description": "tdd ÔåÆ implementation: present in .st3/workflows.yaml (H rename)",
              "validates": {
                "type": "contains_text",
            ...
            {
              "id": "D1.3",
              "description": "GitConfig.extract_issue_number() implemented (I3 ÔÇö cohesion in GitConfig)",
              "validates": {
                "type": "contains_text",
        ...
        },
        {
          ...
          "deliverables": [
            ...
            {
              "id": "D2.2",
              "description": "BranchState frozen=True ÔÇö CQS enforced at type-system level (E)",
              "validates": {
                "type": "contains_text",
                "file": "mcp_server/managers/state_repository.py",
                "text": "frozen=True"
            ...
            {
              "id": "D2.6",
              "description": "PSE constructor receives IStateRepository ÔÇö no direct file I/O in PSE (DIP, E)",
              "validates": {
                "type": "contains_text",
                "file": "mcp_server/managers/phase_state_engine.py",
        ...
        },
        {
          ...
          "deliverables": [
            {
              "id": "D3.1",
              "description": "phase_contracts.yaml config file created in .st3/config/ (A ÔÇö Config-First split)",
              "validates": {
                "type": "file_exists",
            ...
            {
              "id": "D3.2",
              "description": "PhaseContractResolver implemented ÔÇö SRP, no StateRepository dependency (D)",
              "validates": {
                "type": "file_exists",
            ...
            {
              "id": "D3.5",
              "description": "PhaseConfigContext facade ÔÇö single injection point for tool layer (D5, G)",
              "validates": {
                "type": "contains_text",
                "file": "mcp_server/managers/phase_contract_resolver.py",
        ...
        },
        {
          ...
          "deliverables": [
            {
              "id": "D4.1",
              "description": "PSE.get_state(branch) ÔåÆ BranchState added ÔÇö composition root (J1)",
              "validates": {
                "type": "contains_text",
                "file": "mcp_server/managers/phase_state_engine.py",
                "text": "def get_state"
        ...
        },
        {
          ...
          "deliverables": [
            {
              "id": "D5.1",
              "description": "enforcement.yaml config file created in .st3/config/ (F ÔÇö enforcement rules)",
              "validates": {
                "type": "file_exists",
            ...
            {
              "id": "D5.2",
              "description": "EnforcementRunner service implemented ÔÇö SRP, no PSE dependency (F)",
              "validates": {
                "type": "file_exists",
            ...
            {
              "id": "D5.3",
              "description": "BaseTool.enforcement_event class variable added ÔÇö declarative hook registration (F, Option C)",
              "validates": {
                "type": "contains_text",
            ...
            {
              "id": "D5.4",
              "description": "EnforcementRegistry referenced in enforcement_runner.py ÔÇö plugin fail-fast at startup (F2)",
              "validates": {
                "type": "contains_text",
                "file": "mcp_server/managers/enforcement_runner.py",
        ...
        },
        {
          ...
          "deliverables": [
            {
              "id": "D6.1",
              "description": "project_manager.py writes to deliverables.json ÔÇö projects.json abolished (B, C)",
              "validates": {
                "type": "contains_text",
            ...
            {
              "id": "D6.4",
              "description": "state.json removed from .gitignore ÔÇö git-tracked per branch (B5)",
              "validates": {
                "type": "absent_text",
                "file": ".gitignore",
                "text": ".st3/state.json"
          ...
          "exit_criteria": "Completed-cycle guard raises ValidationError with cycle id; AtomicJsonWriter used for all deliverables.json writes (grep: no direct open() calls in project_manager.py for deliverables); integration test: full transition_phase flow commits state.json automatically; Pyright --strict passes on save/update tools and project_manager.py; full test suite green; .st3/state.json restored to implementation phase state before PR/merge work; KPIs 1ÔÇô20 in research_config_first_pse.md all verifiable"
        }
      ]
    },
    "design": {
      "deliverables": [
        ...
        {
          "id": "DD.2",
          "description": "design.md contains Key Design Decisions table for AÔÇôJ",
          "validates": {
            "type": "contains_text",
            "file": "docs/development/issue257/design.md",
    ...
    "validation": {
      "deliverables": [
        {
          "id": "DV.1",
          "description": "KPI section present in research_config_first_pse.md ÔÇö 20 KPIs verifiable after Cycle 6",
          "validates": {
            "type": "contains_text",
    ...
    "documentation": {
      "deliverables": [
        {
          "id": "DDOC.1",
          "description": "planning.md SCAFFOLD header present ÔÇö template-tracked artifact",
          "validates": {
            "type": "contains_text",
...
- **[design.md ÔÇö Config-First PSE Architecture (decisions AÔÇôJ, interfaces, component diagram)][related-1]**
- **[research_config_first_pse.md ÔÇö Research source + KPIs 1ÔÇô20 (frozen)][related-2]**
- **[../../coding_standards/ARCHITECTURE_PRINCIPLES.md ÔÇö Binding architecture contract][related-3]**
- **[../../coding_standards/QUALITY_GATES.md ÔÇö Gate 7: architectural review checklist][related-4]**

<!-- Link definitions -->

[related-1]: design.md ÔÇö Config-First PSE Architecture (decisions AÔÇôJ, interfaces, component diagram)
[related-2]: research_config_first_pse.md ÔÇö Research source + KPIs 1ÔÇô20 (frozen)
[related-3]: ../../coding_standards/ARCHITECTURE_PRINCIPLES.md ÔÇö Binding architecture contract
[related-4]: ../../coding_standards/QUALITY_GATES.md ÔÇö Gate 7: architectural review checklist
