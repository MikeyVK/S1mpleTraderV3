...
class AsyncEventBus:
    ...
    async def publish(
        ...
        if self._event_count % 10 == 0:
            ...
            print(f"[EventBus] Published {self._event_count} events (avg publish time: {avg_time*1000:.2f}ms)")
    ...
    def _should_receive(
        ...
        event_strategy_id: Optional[str]
    ) -> bool:
...
class OhlcvProvider:
    ...
    def __init__(self, event_bus: AsyncEventBus, symbol: str = "BTC/USD"):
...
class StrategyWorker:
    ...
    async def _process_event(self, event: object) -> Optional[SignalDetectedEvent]:
        """
...
async def main():
    ...
    # Run for 10 seconds
    print(f"Running demo for 10 seconds...")
    print(f"Watch for:")
    print(f"  - Platform events published every 0.5-3 seconds")
    print(f"  - Workers processing events from queues")
    print(f"  - Signals detected and monitored")
    print(f"  - NO events missed despite CPU work")
    print()
