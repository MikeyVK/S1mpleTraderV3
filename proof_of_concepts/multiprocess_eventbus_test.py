...
class AsyncEventBus:
    ...
    async def run(self, processing_delay_ms: int = 0):
        """
        ...
        while self._sources_completed < self._total_sources:
            ...
            try:
                ...
                # Process event
                elapsed = time.perf_counter() - start_time
                print(f"  [{elapsed:.3f}s] EventBus: Received {event.source} Event_{event.event_id}")
...
class SlowConsumer:
    ...
    async def run(self, processing_time_ms: int = 100):
        """
