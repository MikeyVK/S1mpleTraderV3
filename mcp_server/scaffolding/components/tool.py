...
class ToolScaffolder(BaseScaffolder):
    ...
    def scaffold(self, name: str, **kwargs: Any) -> str:  # noqa: ANN401
        ...
        try:
            return str(self.renderer.render("components/tool.py.jinja2", name=tool_name, **kwargs))
        except Exception as e:
            # Fallback to generic component template
            if "not found" in str(e).lower():
                return str(
                    self.renderer.render("components/generic.py.jinja2", name=tool_name, **kwargs)
                )
            raise
