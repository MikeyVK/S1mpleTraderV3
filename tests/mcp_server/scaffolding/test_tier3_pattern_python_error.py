...
# template=unit_test version=6b0f1f7e created=2026-02-01T14:11Z updated=
"""
Unit tests for mcp_server.scaffolding.renderer.

Tests the tier3_pattern_python_error.jinja2 macro library (Tier 3 pattern).

@layer: Tests (Unit)
@dependencies: [pytest, mcp_server.scaffolding.renderer]
@responsibilities:
    - Test TestTier3PatternPythonError functionality
    - Verify template structure and required error-handling macros
...
class TestTier3PatternPythonError:
    ...
    def test_template_exists(
        self    ):
        """Verify tier3_pattern_python_error.jinja2 exists and loads.."""
        ...
        template = env.get_template("tier3_pattern_python_error.jinja2")
    ...
    def test_macros_render_expected_tokens(
        self    ):
        """Verify error macros render expected tokens.."""
        # Arrange - Setup test data and preconditions
        from jinja2 import Environment, FileSystemLoader
        from pathlib import Path
        ...
        raise_rendered = template.module.pattern_error_raise(exc_class="WorkerInitializationError", message_key="worker.init_failed")
        wrap_rendered = template.module.pattern_error_wrap(exc_class="WorkerInitializationError", message_key="worker.init_failed")
