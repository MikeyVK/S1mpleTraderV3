...
# template=unit_test version=6b0f1f7e created=2026-02-01T14:16Z updated=
"""
Unit tests for mcp_server.scaffolding.renderer.

Tests the tier3_pattern_python_di.jinja2 macro library (Tier 3 pattern).

@layer: Tests (Unit)
@dependencies: [pytest, mcp_server.scaffolding.renderer]
@responsibilities:
    - Test TestTier3PatternPythonDi functionality
    - Verify template structure and required DI macros
...
class TestTier3PatternPythonDi:
    ...
    def test_template_exists(
        self    ):
        """Verify tier3_pattern_python_di.jinja2 exists and loads.."""
        ...
        template = env.get_template("tier3_pattern_python_di.jinja2")
    ...
    def test_macros_render_expected_tokens(
        self    ):
        """Verify DI macros render expected tokens.."""
        # Arrange - Setup test data and preconditions
        from jinja2 import Environment, FileSystemLoader
        from pathlib import Path
        ...
        get_rendered = template.module.pattern_di_get_capability(attr_name="_persistence", cap_key="persistence")
        req_rendered = template.module.pattern_di_require_capability(cap_key="persistence", message_key="worker.missing_capability")
