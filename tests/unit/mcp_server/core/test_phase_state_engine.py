...
class TestPhaseStateEngineGetPhase:
    ...
    def test_get_phase_returns_current_phase(self, feature_phases):
        """Should return current phase for tracked branch."""
        engine = PhaseStateEngine(workspace_root=Path("."))
        first_phase = feature_phases[0]
        # Setup: initialize branch with phase
        engine.initialize_branch("feature/31-test", first_phase, issue_number=31)

        phase = engine.get_phase("feature/31-test")

        assert phase == first_phase

    def test_get_phase_after_transition(self, feature_phases):
        """Should return updated phase after transition."""
        engine = PhaseStateEngine(workspace_root=Path("."))
        first_phase = feature_phases[0]
        second_phase = feature_phases[1]
        engine.initialize_branch("feature/31-test", first_phase, issue_number=31)
        engine.transition("feature/31-test", first_phase, second_phase)
...
class TestPhaseStateEngineInitializeBranch:
    ...
    def test_initialize_branch_creates_state(self, feature_phases):
        """Should create initial state for new branch."""
        engine = PhaseStateEngine(workspace_root=Path("."))
        first_phase = feature_phases[0]

        result = engine.initialize_branch(
            "feature/31-test", first_phase, issue_number=31
        )

        assert result is True
        assert engine.get_phase("feature/31-test") == first_phase

    def test_initialize_branch_stores_issue_mapping(self, feature_phases):
    ...
    def test_initialize_branch_fails_if_already_exists(self, feature_phases):
        ...
        result = engine.initialize_branch(
            "feature/31-test", second_phase, issue_number=31
        )
...
class TestPhaseStateEngineTransition:
    ...
    def test_transition_valid_phase_change(self, feature_phases):
        ...
        result = engine.transition("feature/31-test", first_phase, second_phase)

        assert result.success is True
        assert result.new_phase == second_phase
        assert engine.get_phase("feature/31-test") == second_phase

    def test_transition_records_history(self, feature_phases):
        ...
        assert history[-1]["from_phase"] == first_phase
        assert history[-1]["to_phase"] == second_phase

    def test_transition_with_wrong_from_phase_fails(self, feature_phases):
        """Should fail if from_phase doesn't match current."""
        engine = PhaseStateEngine(workspace_root=Path("."))
        first_phase = feature_phases[0]
        second_phase = feature_phases[1]
        third_phase = feature_phases[2]
        engine.initialize_branch("feature/31-test", first_phase, issue_number=31)

        result = engine.transition("feature/31-test", second_phase, third_phase)

        assert result.success is False
        assert "Phase mismatch" in result.reason

    def test_transition_for_unknown_branch_fails(self, feature_phases):
        """Should fail for branch without state."""
        engine = PhaseStateEngine(workspace_root=Path("."))
        first_phase = feature_phases[0]
        second_phase = feature_phases[1]

        result = engine.transition("feature/unknown", first_phase, second_phase)

        assert result.success is False
        assert "not initialized" in result.reason.lower()

    def test_transition_with_human_approval(self, feature_phases):
...
class TestPhaseStateEngineGetProjectPlan:
    ...
    def test_get_project_plan_for_branch(self, feature_phases):
...
class TestPhaseStateEnginePersistence:
    ...
    def test_state_persists_across_instances(self, feature_phases):
        """Should persist state to file and reload."""
        workspace = Path(".")
        first_phase = feature_phases[0]
        second_phase = feature_phases[1]
        engine1 = PhaseStateEngine(workspace_root=workspace)
        engine1.initialize_branch("feature/31-test", first_phase, issue_number=31)
        engine1.transition("feature/31-test", first_phase, second_phase)
    ...
    def test_state_file_created_in_st3_directory(self, feature_phases):
