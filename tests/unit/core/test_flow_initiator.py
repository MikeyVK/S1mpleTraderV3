...
class TestFlowInitiatorDataHandling:
    ...
    def flow_initiator(self, cache_mock: Mock) -> FlowInitiator:
        ...
        flow_initiator.initialize(
            strategy_cache=cache_mock,
            dto_types={
                "candle_stream": MockCandleWindow,
                "news_feed": MockNewsEvent
            }
        )
...
class TestFlowInitiatorErrorHandling:
    ...
    def test_on_data_ready_raises_on_unknown_source_type(
        self,
        flow_initiator: FlowInitiator,
        test_timestamp: datetime
    ) -> None:
        """on_data_ready raises ValueError for unknown source_type."""
        platform_dto = PlatformDataDTO(
            source_type="unknown_type",  # Not in dto_types mapping!
            timestamp=test_timestamp,
            payload=MockCandleWindow(symbol="BTC_EUR", data="test")
        )
    ...
    def test_error_message_shows_available_types(
        ...
        """Error message for unknown source_type shows available types."""
        platform_dto = PlatformDataDTO(
