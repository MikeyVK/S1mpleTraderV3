...
class TestTemplateRootConfiguration:
    ...
    def test_default_uses_tier_root(self):
        """When no config set, should use mcp_server/scaffolding/templates as default."""
        scaffolder = TemplateScaffolder()
        
        # Expected: mcp_server/scaffolding/templates (tier-root)
        expected_tier_root = Path("mcp_server/scaffolding/templates")
        actual_root = scaffolder._renderer.template_dir
    ...
    def test_fail_fast_on_nonexistent_path(self):
        """When configured path doesn't exist, should raise ConfigError immediately."""
    ...
    def test_no_fallback_to_legacy_templates_dir(self):
        """Should NEVER fall back to mcp_server/templates (legacy location)."""
        scaffolder = TemplateScaffolder()
    ...
    def test_validation_service_uses_same_root(self):
        """ValidationService should use the same template root as scaffolder."""
        from mcp_server.validation.validation_service import ValidationService
        
        scaffolder = TemplateScaffolder()
        validation_service = ValidationService()
        
        assert scaffolder._renderer.template_dir == validation_service.template_analyzer.template_root, (
