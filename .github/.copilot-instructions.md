...
## 🏛️ Architectuurcontract (VERPLICHT — lees bij elke implementatietaak)

**Voordat je implementatiecode schrijft, lees:**
**[docs/coding_standards/ARCHITECTURE_PRINCIPLES.md](../docs/coding_standards/ARCHITECTURE_PRINCIPLES.md)**

Dit document is een **bindend contract**. Code die deze principes schendt wordt **REJECTED**, ongeacht of tooling-gates groen zijn.

### Meest voorkomende schendingen (snel-referentie):

| Schending | Correct patroon |
|---|---|
| Hardcoded fase-/workflow-namen in Python | Lees uit `WorkflowConfig` / `GitConfig` |
| `SomeManager()` binnen `execute()` | Constructor-injectie via `__init__` |
| `IStateRepository` voor read-only consumer | Gebruik `IStateReader` (ISP) |
| `get_state()` roept `save()` aan | CQS-schending — split de methode |
| Module-level `Config.load()` | `ClassVar` + lazy init in `__init__` |
| if-chain op `phase == "tdd"` etc. | Registry of config-dispatch (OCP) |
| Value object zonder `frozen=True` | Voeg `@dataclass(frozen=True)` toe |
| `_extract_issue_from_branch()` in PSE | Gebruik `GitConfig.extract_issue_number()` |
